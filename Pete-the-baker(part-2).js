...
function getMissingIngredients(recipe, added) {
  ...
  for (let ingredient in recipe) {
    if (ingredient in added)
      maxCakes = Math.max(
        Math.round(added[ingredient] / recipe[ingredient]),
        maxCakes
      );
  }
  ...
  for (let ingredient in recipe) {
    if (!(ingredient in added))
      result[ingredient] = maxCakes * recipe[ingredient];
    else if (added[ingredient] < maxCakes * recipe[ingredient])
      result[ingredient] = maxCakes * recipe[ingredient] - added[ingredient];
  }
  return result;
}
