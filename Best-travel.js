...
//    t - maximum sum of distances, integer >= 0 (174 miles)
...
function chooseBestSum(t, k, ls) {
  ...
  for (const value of ls) {
    // create copy of results on each iteration
    const copy = [...results];

    // get array inside of 2d copy array
  ...
  return (
    results
      // filter out arrays that are required length
      .filter((arr) => arr.length == k)
      // map over each array and return new array where each individual arrays items are added together
      .map((item) => item.reduce((a, b) => a + b), 0)
      // filter for items that are <= total distance allowed
      .filter((item) => item <= t)
      // order array from high to low and return highest (index 0)
      .sort((a, b) => b - a)[0] || null
  );
}
